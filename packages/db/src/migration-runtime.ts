...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://fightforge:fightforge@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "fightforge");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://fightforge:fightforge@127.0.0.1:${preferredPort}/fightforge`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://fightforge:fightforge@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "fightforge");
    return {
      connectionString: `postgres://fightforge:fightforge@127.0.0.1:${port}/fightforge`,
  ...
  const adminConnectionString = `postgres://fightforge:fightforge@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "fightforge");

  return {
    connectionString: `postgres://fightforge:fightforge@127.0.0.1:${selectedPort}/fightforge`,
