...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "fightforge-embedded-postgres-probe-"));
...
export async function startEmbeddedPostgresTestDatabase(
  ...
  try {
    ...
    const connectionString = `postgres://fightforge:fightforge@127.0.0.1:${port}/fightforge`;
