...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "fightforge-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveFightForgeHomeDir(): string {
  const envHome = process.env.FIGHTFORGE_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".fightforge");
}

function resolveFightForgeInstanceId(): string {
  const raw = process.env.FIGHTFORGE_INSTANCE_ID?.trim() || DEFAULT_INSTANCE_ID;
  if (!INSTANCE_ID_RE.test(raw)) {
    throw new Error(`Invalid FIGHTFORGE_INSTANCE_ID '${raw}'.`);
  }
  return raw;
}
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveFightForgeHomeDir(), "instances", resolveFightForgeInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".fightforge", CONFIG_BASENAME);
...
function resolveFightForgeConfigPath(): string {
...
function resolveFightForgeEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  const configPath = resolveFightForgeConfigPath();
  const envPath = resolveFightForgeEnvPath(configPath);
