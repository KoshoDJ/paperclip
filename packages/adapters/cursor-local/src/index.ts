...
Use when:
- You want FightForge to run Cursor Agent CLI locally as the agent runtime
...
- FightForge auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$fightforge" and related skills on local runs.
- FightForge auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
