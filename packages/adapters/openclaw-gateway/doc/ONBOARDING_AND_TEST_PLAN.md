...
## Scope
This plan is now **gateway-only**. FightForge supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses FightForge skill.
...
```json
{
  "fightforgeApiUrl": "http://host.docker.internal:3100",
...
3. In `/new` OpenClaw session, OpenClaw can still create a FightForge task.
...
```bash
pnpm --filter @fightforgeai/server test:run -- openclaw-gateway
pnpm --filter @fightforgeai/server typecheck
pnpm --filter @fightforgeai/ui typecheck
pnpm --filter fightforgeai typecheck
```
