...
This document describes how `@fightforgeai/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into FightForge logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (FightForge `runId`)
