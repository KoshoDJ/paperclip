...
import { asString, parseObject } from "@fightforgeai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the FightForge server host.",
        });
      }
    } catch (err) {
      checks.push({
