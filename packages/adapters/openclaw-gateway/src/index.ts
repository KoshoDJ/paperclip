...
Use when:
- You want FightForge to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the FightForge server.
...
- fightforgeApiUrl (string, optional): absolute FightForge base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default fightforge)

Standard outbound payload additions:
- fightforge (object): standardized FightForge context added to every gateway agent request
- fightforge.workspace (object, optional): resolved execution workspace for this run
- fightforge.workspaces (array, optional): additional workspace hints FightForge exposed to the run
- fightforge.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
