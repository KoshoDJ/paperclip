...
export interface FightForgeSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: FightForgeSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "fightforge_required",
      originLabel: "Required by FightForge",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by FightForge",
    readOnly: false,
  };
}
...
export function buildFightForgeEnv(agent: { id: string; companyId: string }): Record<string, string> {
...
export async function resolveFightForgeSkillsDir(
  moduleDir: string,
...
export async function listFightForgeSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<FightForgeSkillEntry[]> {
  const root = await resolveFightForgeSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `fightforgeai/fightforge/${entry.name}`,
        runtimeName: entry.name,
        source: path.join(root, entry.name),
        required: true,
        requiredReason: "Bundled FightForge skills are always available for local adapters.",
      }));
  } catch {
    return [];
  }
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the FightForge skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "FightForge cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredFightForgeRuntimeSkills(value: unknown): FightForgeSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: FightForgeSkillEntry[] = [];
...
export async function readFightForgeRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<FightForgeSkillEntry[]> {
  const configuredEntries = normalizeConfiguredFightForgeRuntimeSkills(config.fightforgeRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listFightForgeSkillEntries(moduleDir, additionalCandidates);
}

export async function readFightForgeSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listFightForgeSkillEntries(moduleDir);
...
export function readFightForgeSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.fightforgeSkillSync;
...
function canonicalizeDesiredFightForgeSkillReference(
  reference: string,
...
export function resolveFightForgeDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readFightForgeSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredFightForgeSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeFightForgeSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.fightforgeSkillSync;
  const current =
  ...
  next.fightforgeSkillSync = current;
  return next;
}

export async function ensureFightForgeSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the FightForge server itself is started from
    // within a Claude Code session (e.g. `npx fightforgeai run` in a terminal
