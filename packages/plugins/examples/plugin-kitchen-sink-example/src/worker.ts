...
import {
  ...
  type ToolResult,
  type ToolRunContext,
} from "@fightforgeai/plugin-sdk";
import type { Goal, Issue } from "@fightforgeai/shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.companySummary,
    {
      ...
      description: "Summarizes current company counts from the FightForge APIs.",
...
const plugin: FightForgePlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
