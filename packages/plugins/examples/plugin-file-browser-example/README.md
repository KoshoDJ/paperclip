...
Example FightForge plugin that demonstrates:
...
```bash
pnpm --filter @fightforgeai/plugin-file-browser-example build
pnpm fightforgeai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm fightforgeai plugin uninstall fightforge-file-browser-example --force
```
...
- Optional: use `fightforge-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
