...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".fightforge-sdk");
...
/**
 * Generate a complete FightForge plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A FightForge plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "fightforge-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["fightforge", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@fightforgeai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@fightforgeai/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@fightforgeai/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@fightforgeai/plugin-sdk\` and \`@fightforgeai/shared\` from a local FightForge checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.fightforge-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into FightForge
...
- \`pnpm build\` uses esbuild presets from \`@fightforgeai/plugin-sdk/bundlers\`.
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-fightforge-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <fightforge-sdk-path>]");
    process.exit(1);
  }
