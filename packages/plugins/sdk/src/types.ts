/**
 * Core types for the FightForge plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@fightforgeai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @fightforgeai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of FightForge object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the FightForge UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit FightForge domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core FightForge domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the FightForge secret provider
...
export interface PluginSecretsClient {
   ...
   * in the FightForge secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@fightforgeai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@fightforgeai/plugin-sdk";
 *
