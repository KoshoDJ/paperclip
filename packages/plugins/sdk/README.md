# `@fightforgeai/plugin-sdk`

Official TypeScript SDK for FightForge plugin authors.

- **Worker SDK:** `@fightforgeai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@fightforgeai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@fightforgeai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@fightforgeai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@fightforgeai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@fightforgeai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@fightforgeai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@fightforgeai/plugin-sdk/ui/hooks` | Hooks only |
| `@fightforgeai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@fightforgeai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@fightforgeai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@fightforgeai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@fightforgeai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@fightforgeai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @fightforgeai/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main FightForge app. They can call ordinary FightForge HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the FightForge instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@fightforgeai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@fightforgeai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core FightForge information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@fightforgeai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@fightforgeai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@fightforgeai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@fightforgeai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@fightforgeai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@fightforgeai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@fightforgeai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@fightforgeai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@fightforgeai/plugin-sdk/ui";
...
import {
  useHostContext,
  usePluginAction,
} from "@fightforgeai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@fightforgeai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@fightforgeai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@fightforgeai/plugin-sdk/bundlers";
...
```bash
fightforge-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@fightforgeai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__fightforge__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__fightforge__/events` streams `reload` SSE events on UI build changes
