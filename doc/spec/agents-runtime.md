...
Audience: Operators setting up and running agents in FightForge

## 1. What this system does

Agents in FightForge do not run continuously.  
...
For `claude_local` and `codex_local`, FightForge assumes the CLI is already installed and authenticated on the host machine.
...
FightForge stores resumable session state per `(agent, taskKey, adapterType)`.
...
FightForge pushes runtime/activity updates to the browser in real time.
