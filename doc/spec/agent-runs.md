...
This spec defines how FightForge actually runs agents while staying runtime-agnostic.
...
1. FightForge is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, FightForge manages lifecycle, and on exit FightForge parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `fightforge.skill` (shared FightForge skill text block)
