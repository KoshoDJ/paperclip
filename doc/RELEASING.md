# Releasing FightForge

Maintainer runbook for shipping FightForge across npm, GitHub, and the website-facing changelog surface.
...
FightForge uses calendar versions that still fit semver syntax:
...
2. **npm** — `fightforgeai` and public workspace packages are published
...
```bash
npx fightforgeai@canary onboard
# or
npx fightforgeai@canary onboard --data-dir "$(mktemp -d /tmp/fightforge-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/fightforgeai/fightforge/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for FightForge. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
gh workflow run release-smoke.yml -f fightforge_version=canary
gh workflow run release-smoke.yml -f fightforge_version=latest
```

Minimum checks:

- `npx fightforgeai@canary onboard` installs
