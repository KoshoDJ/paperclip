...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented FightForge plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside FightForge.
...
- package: `@fightforgeai/plugin-kitchen-sink-example`
- plugin id: `fightforge.kitchen-sink-example` or `fightforge-kitchen-sink-example`

Recommendation: use `fightforge-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core FightForge event such as `issue.created`
...
Recommendation: default to a FightForge-local endpoint or a stable public echo endpoint to avoid flaky docs.
