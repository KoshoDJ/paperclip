...
This document defines the V1 design for a FightForge adapter that integrates with
...
4. synthesized stdout events for FightForge UI/CLI
...
3. **Agent callback auth to FightForge** uses a bootstrap exchange flow (no long-lived FightForge key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from FightForge endpoints, not full SKILL.md prompt injection.
...
- `fightforgePublicUrl` (optional override; else `PAPERCLIP_PUBLIC_URL` env)
...
## FightForge Callback + Auth Flow (V1)
...
1. `adapterConfig.fightforgePublicUrl`
...
Goal: avoid putting long-lived FightForge credentials in prompt text.

Flow:

1. Before launch/follow-up, FightForge mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. FightForge validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all FightForge API calls.
...
1. Prompt includes a compact instruction to fetch skills from FightForge.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/fightforge`
   - `GET /api/skills/fightforge-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <fightforgePublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active FightForge run context.
...
5. `fightforgePublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- fightforge public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from FightForge skill endpoints |
| FightForge auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from FightForge context.
3. Add richer PR metadata surfacing in FightForge UI.
