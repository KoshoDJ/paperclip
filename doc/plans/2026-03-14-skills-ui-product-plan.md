...
This document defines the product and UI plan for skill management in FightForge.
...
- FightForge company import/export can include skills as package content
...
3. The UI must always tell the truth about what FightForge knows:
   - desired state in FightForge
...
FightForge should treat skills at two scopes:
...
- created locally inside FightForge later
...
7. “Understand what FightForge knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a FightForge-specific package layout
...
This matters because Codex and similar adapters may already have local skills that FightForge did not install.

These should be clearly marked:

- external
- not managed by FightForge
...
- `desired_only`: FightForge wants it, adapter does not show it yet
- `external`: adapter has it but FightForge does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, FightForge only tracks desired state
...
- FightForge can still track desired skills
...
- FightForge can see adapter skills
...
- `.fightforge.yaml` may add FightForge-specific fidelity, but should not replace the base shortname association model
...
- `.fightforge.yaml` may add FightForge-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
FightForge should have a company-scoped skill model or managed package model representing:
...
FightForge should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too FightForge-specific.
...
That gives FightForge one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
