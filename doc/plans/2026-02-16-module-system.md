...
FightForge's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends FightForge's API, UI, and data model | Yes |
...
modules/
  observability/
    fightforge.module.json     # manifest (required)
    src/
...
### Manifest (`fightforge.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@fightforgeai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@fightforgeai/core";
...
import type { Db } from "@fightforgeai/db";
...
```
1. Scan modules/ directory for fightforge.module.json manifests
...
```jsonc
// fightforge.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@fightforgeai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@fightforgeai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:fightforge/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.fightforge.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm fightforgeai store list                    # browse available modules and templates
pnpm fightforgeai store install <module-id>     # install a module
pnpm fightforgeai store import <template-id>    # import a company template
pnpm fightforgeai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one FightForge instance | `server:started` |
...
4. **Module config** — `fightforge.config.json` with per-module config, env var interpolation
...
10. **`@fightforgeai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
