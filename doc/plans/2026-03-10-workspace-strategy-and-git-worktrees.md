...
`PAP-447` asks how FightForge should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same FightForge instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into FightForge:

- some operators use FightForge to manage FightForge and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as FightForge
- Claude and Codex expose different built-in affordances, so FightForge should not overfit to one tool

## Core Product Decision

FightForge should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by FightForge
...
- FightForge should not assume that every adapter can see or use a host filesystem path directly
...
If FightForge makes worktrees agent-first, it will blur:
...
- FightForge core resolves and records execution workspace state
...
That gives FightForge room to support:
...
- FightForge handles workspace runtime services under the hood using project/workspace policy
...
Once FightForge is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So FightForge should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that FightForge cannot infer
...
- when issue moves to done, should FightForge auto-commit?
- should it auto-push?
...
FightForge already has the right foundation for a project-first model.
...
For server-spawned local adapters, FightForge already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that FightForge should mirror directly

Implication:

- for `codex_local`, FightForge should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but FightForge should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, FightForge should separate:
...
FightForge should be able to express intentions such as:
...
These are not specific to FightForge. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".fightforge/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a FightForge-specific `devServer` flag.
...
FightForge should distinguish between:
...
FightForge should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, FightForge should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to FightForge
...
For runtime services specifically, FightForge should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as FightForge-only dev-server flags
...
3. Inject resolved execution workspace details into `context.fightforgeWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / FightForge instance can be reused through env/config injection when appropriate
...
- FightForge can decide whether to reuse or start a fresh service deterministically
...
3. Test against FightForge-on-FightForge workflows first.
...
7. Workspace runtime services are modeled generically, not as FightForge-specific dev-server toggles.
