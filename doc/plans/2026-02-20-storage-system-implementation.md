...
Add a single storage subsystem for FightForge that supports:
...
- API serves attachment bytes through authenticated FightForge endpoints.
...
- Default local path is under instance root: `~/.fightforge/instances/<instanceId>/data/storage`.
...
- `fightforgeai onboard` writes a valid `storage` config block by default.
- `fightforgeai configure --section storage` can switch between local and s3 modes.
...
- `fightforgeai doctor` reports actionable storage status.
