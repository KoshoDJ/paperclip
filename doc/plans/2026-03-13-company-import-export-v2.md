...
This document defines the next-stage plan for FightForge company import/export.

The core shift is:

- move from a FightForge-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside FightForge.
...
FightForge already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just FightForge
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `fightforge.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.fightforge.yaml` sidecar for high-fidelity FightForge-specific details
...
11. adapter-aware skill sync surfaces so FightForge can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point FightForge at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a FightForge sidecar.
...
FightForge must not redefine `SKILL.md`.
...
- FightForge-specific fidelity lives in `.fightforge.yaml`
- FightForge may resolve and install `SKILL.md` packages, but it must not require a FightForge-only skill format
...
### 5.4 Base Package Vs FightForge Extension
...
- base package:
  ...
  - no FightForge-only runtime fields by default
- FightForge extension:
  - `.fightforge.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by FightForge tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`fightforge.manifest.json` is not part of the future package direction.
...
FightForge import/export should support these entity kinds:
...
- FightForge can treat “this set of agents came from team package X” as the imported-team model
...
FightForge should support source references in package metadata with:
...
FightForge should define a new adapter surface area around skills:
...
Planned FightForge behavior:

- if an adapter supports read, FightForge should show current skills in the UI
- if an adapter supports write, FightForge should let the user enable/disable imported skills
- if an adapter supports sync, FightForge should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.fightforge.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.fightforge.yaml` alongside the base package
...
## 10. Storage Model Inside FightForge
...
FightForge should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `fightforgeai company export <company-id> --out <path>`
- `fightforgeai company import <path-or-url> --dry-run`
- `fightforgeai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in FightForge storage, or only referenced at import time?
...
3. Should FightForge support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps FightForge aligned with:
