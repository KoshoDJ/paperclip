...
This document defines the rollout plan for adapter-wide skill support in FightForge.
...
- FightForge stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

FightForge currently has these adapters:
...
1. Desired skills live in FightForge for every adapter.
...
These adapters have a stable local skills directory that FightForge can read and manage.
...
These adapters do not have a meaningful FightForge-owned persistent install state.
...
- show desired FightForge skills
...
- desired skills stored in FightForge
...
- runtime already injects FightForge skills into `~/.cursor/skills`
...
- runtime already injects FightForge skills into `~/.gemini/skills`
...
- runtime already injects FightForge skills into `~/.pi/agent/skills`
...
- FightForge can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects FightForge skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- FightForge may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed FightForge skills that are clearly marked as FightForge-managed
...
- FightForge stores desired skills only
...
- avoid destructive wording unless FightForge can prove a skill is FightForge-managed
...
That gets FightForge from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
