...
FightForge needs to support two very different but equally valid ways of working:
...
Today, FightForge already has the beginnings of this model:
...
- making FightForge feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing FightForge users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted FightForge deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning FightForge into a full code review product
...
This object must be recorded explicitly so that FightForge can:
...
FightForge should treat PRs as a type of work product linked back to:
...
FightForge already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted FightForge must be a first-class deployment mode

FightForge cannot assume that it is running on the same machine as the code.

In cloud deployments, FightForge may:
...
- `Runtime service`: a process or service FightForge owns or tracks for a workspace
...
It also lets cloud-hosted FightForge deployments point at codebases and remotes without pretending the FightForge host has direct filesystem access.
...
Without an explicit `execution workspace` record, FightForge has nowhere stable to attach:
...
FightForge needs a single place to show:
...
- FightForge-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without FightForge owning the execution host
...
## 1. FightForge host and execution host must be decoupled
...
without the FightForge host itself running local git or local preview processes.
...
FightForge should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for FightForge.
...
FightForge should keep workspace automation centered on a small set of hooks:
...
FightForge should provide the equivalent visibility in the project `Code` surface:
...
FightForge should treat execution workspaces as the natural home for:
...
FightForge should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
FightForge should continue treating cleanup policy as part of the core workspace model:
...
FightForge should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the FightForge host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

FightForge should borrow Worktrunk's good execution patterns while keeping the broader FightForge model:
...
This is how FightForge supports:
...
That keeps FightForge focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted FightForge deployments remain first-class

That is a better fit for FightForge than either extreme:
