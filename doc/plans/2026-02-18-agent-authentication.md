...
Agents need API keys to authenticate with FightForge. The current approach
...
**Trust model:** The adapter process runs on the same machine as the FightForge
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** FightForge generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a FightForge CLI
...
```
fightforge auth login
# Opens browser -> user confirms -> token stored at ~/.fightforge/credentials
```
...
managed by the FightForge server itself.
...
1. A company admin (user or agent) generates an **invite URL** from FightForge.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The FightForge SKILL.md (or a link to it)
   - What information FightForge needs from the agent (e.g. webhook URL, adapter
...
5. FightForge stores the pending registration.
...
7. On approval, FightForge provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** FightForge issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.fightforge.ing/skills/fightforge/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How FightForge should send you heartbeats",
...
1. Generate a FightForge invite link for the company.
...
4. A FightForge company member approves the new agent.
5. FightForge begins sending heartbeats to the OpenClaw webhook endpoint.
...
| **P3**   | CLI auth flow                     | `fightforgeai auth login` for developer-managed remote agents.                                      |
