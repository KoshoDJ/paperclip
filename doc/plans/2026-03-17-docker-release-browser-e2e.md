...
Today release smoke testing for published FightForge packages is manual and shell-driven:
...
- Docker
- `npx fightforgeai@canary`
- `npx fightforgeai@latest`
...
- runs `fightforgeai@${PAPERCLIPAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@fightforge.local`
- password: `fightforge-smoke-password`
...
1. A published `fightforgeai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `fightforgeai@latest` Docker install can be smoke-tested by Playwright in CI.
