...
ClipHub is the public registry where people share, discover, and download FightForge company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to FightForge what a package registry is to a programming language. FightForge already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in FightForge — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own FightForge instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the FightForge spec. It contains:
...
**Install (fresh start):**
```
fightforge install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local FightForge instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within FightForge, export your company as a template, then publish:

```
fightforge export --template my-company
fightforge publish cliphub my-company
```
...
ClipHub is a **separate service** from FightForge itself. FightForge is self-hosted; ClipHub is a hosted registry that FightForge instances talk to.
...
| **FightForge CLI** | `fightforgeai install`, `fightforgeai publish`, `fightforgeai cliphub sync` — built into FightForge |
| **FightForge UI** | "Browse ClipHub" panel in the FightForge web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with FightForge) |
| Backend | TypeScript + Hono (consistent with FightForge) |
| Database | PostgreSQL |
...
4. Run `fightforgeai install cliphub:acme/lean-saas-shop`
5. FightForge creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in FightForge until it works well
2. Export: `fightforgeai export --template my-agency`
3. Publish: `fightforgeai publish cliphub my-agency`
...
3. Install just that agent: `fightforgeai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to FightForge

ClipHub is **not required** to use FightForge. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to FightForge what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `fightforgeai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in FightForge web UI
- [ ] `fightforgeai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with FightForge's own philosophy)
