# FightForge Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for FightForge's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main FightForge app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary FightForge HTTP APIs directly.
...
FightForge plugin design is based on the following assumptions:

1. FightForge is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core FightForge business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by FightForge core.
...
2. Let plugins add major capabilities without editing FightForge core.
...
4. Depend on project-local plugin folders such as `.fightforge/plugins`.
...
The single FightForge deployment an operator installs and controls.

### 5.2 Company

A first-class FightForge business object inside the instance.
...
A trusted in-process extension loaded directly by FightForge core.
...
An installable instance-wide extension package loaded through the FightForge plugin runtime.
...
FightForge has two extension classes.
...
FightForge already has a concrete workspace model:
...
Plugins live under the FightForge instance directory.

Suggested layout:

- `~/.fightforge/instances/default/plugins/package.json`
- `~/.fightforge/instances/default/plugins/node_modules/`
- `~/.fightforge/instances/default/plugins/.cache/`
- `~/.fightforge/instances/default/data/plugins/<plugin-id>/`
...
FightForge should add CLI commands:

- `pnpm fightforgeai plugin list`
- `pnpm fightforgeai plugin install <package[@version]>`
- `pnpm fightforgeai plugin uninstall <plugin-id>`
- `pnpm fightforgeai plugin upgrade <plugin-id> [version]`
- `pnpm fightforgeai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@fightforge/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface FightForgePluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumFightForgeVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that FightForge agents can use during runs.
...
- FightForge server starts one worker process per installed plugin
...
Receives one typed FightForge domain event.
...
export function definePlugin(definition: PluginDefinition): FightForgePlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: FightForgePluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@fightforgeai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@fightforgeai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@fightforgeai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@fightforgeai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@fightforgeai/plugin-sdk/ui`
...
The `@fightforgeai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the FightForge secret provider system rather than a plain text input
...
1. Core FightForge data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing FightForge objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual FightForge product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if FightForge later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing FightForge secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm fightforgeai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the FightForge server. This is a normative requirement, not optional.
...
### 27.1 `@fightforgeai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@fightforgeai/plugin-test-harness";
...
For developing a plugin against a running FightForge instance:

- The operator installs the plugin from a local path: `pnpm fightforgeai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-fightforge-plugin`) that scaffolds:

- `package.json` with correct `fightforgePlugin` keys
...
- `@fightforge/plugin-workspace-files`
- `@fightforge/plugin-terminal`
- `@fightforge/plugin-git`
- `@fightforge/plugin-linear`
- `@fightforge/plugin-github-issues`
- `@fightforge/plugin-grafana`
- `@fightforge/plugin-runtime-processes`
- `@fightforge/plugin-stripe`
...
- `@fightforgeai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@fightforgeai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@fightforgeai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@fightforgeai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@fightforgeai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@fightforgeai/plugin-sdk/ui`
...
- `@fightforgeai/plugin-test-harness`
- `create-fightforge-plugin` starter template
...
FightForge should not implement a generic in-process hook bag modeled directly after local coding tools.

FightForge should implement:
...
That is the complete target design for the FightForge plugin system.
