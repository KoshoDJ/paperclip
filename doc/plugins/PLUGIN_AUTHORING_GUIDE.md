...
This guide describes the current, implemented way to create a FightForge plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main FightForge app.
...
```bash
pnpm --filter @fightforgeai/create-fightforge-plugin build
node packages/plugins/create-fightforge-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the FightForge repo:

```bash
pnpm --filter @fightforgeai/create-fightforge-plugin build
node packages/plugins/create-fightforge-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/fightforge/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@fightforgeai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@fightforgeai/plugin-sdk` from the local FightForge checkout into a `.fightforge-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into FightForge from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@fightforgeai/plugin-sdk/ui`
