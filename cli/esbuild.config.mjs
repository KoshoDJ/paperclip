/**
 * esbuild configuration for building the fightforgeai CLI for npm.
 *
 * Bundles all workspace packages (@fightforgeai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @fightforgeai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@fightforgeai/")) {
      externals.add(name);
    }
  }
