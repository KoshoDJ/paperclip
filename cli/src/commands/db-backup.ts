...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@fightforgeai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://fightforge:fightforge@127.0.0.1:${port}/fightforge`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" fightforge db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveFightForgeInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "fightforge";
