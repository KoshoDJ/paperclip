...
const DEFAULT_AGENT_JWT_ISSUER = "fightforge";
const DEFAULT_AGENT_JWT_AUDIENCE = "fightforge-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveFightForgeInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveFightForgeInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" fightforge env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: FightForgeConfig | null = null;
...
function collectDeploymentEnvRows(config: FightForgeConfig | null, configPath: string): EnvVarRow[] {
