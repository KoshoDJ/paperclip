...
} from "@fightforgeai/db";
import type { Command } from "commander";
import { ensureAgentJwtSecret, loadFightForgeEnvFile, mergeFightForgeEnvEntries, readFightForgeEnvEntries, resolveFightForgeEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { FightForgeConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printFightForgeCliBanner } from "../utils/banner.js";
...
const WORKTREE_NAME_PREFIX = "fightforge-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.FIGHTFORGE_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.FIGHTFORGE_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromFightForgeConfig(config: FightForgeConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromFightForgeConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.fightforge${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".fightforge", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".fightforge", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.fightforge"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: FightForgeConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://fightforge:fightforge@127.0.0.1:${port}/fightforge`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: FightForgeConfig;
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: FightForgeConfig;
  targetConfig: FightForgeConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveFightForgeEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readFightForgeEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://fightforge:fightforge@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "fightforge");
    const targetConnectionString = `postgres://fightforge:fightforge@127.0.0.1:${targetHandle.port}/fightforge`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readFightForgeEnvEntries(resolveFightForgeEnvFile(sourceConfigPath));
  ...
  loadFightForgeEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run FightForge inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printFightForgeCliBanner();
  p.intro(pc.bgCyan(pc.black(" fightforgeai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printFightForgeCliBanner();
  p.intro(pc.bgCyan(pc.black(" fightforgeai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasFightForgeConfig: existsSync(path.resolve(worktreePath, ".fightforge", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printFightForgeCliBanner();
  p.intro(pc.bgCyan(pc.black(" fightforgeai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveFightForgeEnvFile(configPath);
  const envEntries = readFightForgeEnvEntries(envPath);
  const out = {
    PAPERCLIP_CONFIG: configPath,
    ...(envEntries.FIGHTFORGE_HOME ? { FIGHTFORGE_HOME: envEntries.FIGHTFORGE_HOME } : {}),
    ...(envEntries.FIGHTFORGE_INSTANCE_ID ? { FIGHTFORGE_INSTANCE_ID: envEntries.FIGHTFORGE_INSTANCE_ID } : {}),
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasFightForgeConfig)
      .map((choice) => path.resolve(choice.worktree, ".fightforge", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readFightForgeEnvEntries(resolveFightForgeEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start FightForge once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasFightForgeConfig ? "fightforge" : "no-fightforge-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".fightforge", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".fightforge", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .fightforge/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasFightForgeConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a FightForge worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasFightForgeConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No FightForge worktrees were found. Run `fightforgeai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target FightForge configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local FightForge instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated FightForge instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with fightforge- if needed (created at ~/fightforge-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: FIGHTFORGE_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: FIGHTFORGE_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source FIGHTFORGE_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local FightForge instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like FightForge worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with fightforge- if needed")
