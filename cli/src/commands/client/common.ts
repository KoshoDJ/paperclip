...
import { ApiRequestError, FightForgeApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to FightForge config file")
    .option("-d, --data-dir <path>", "FightForge data directory root (isolates state from ~/.fightforge)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the FightForge API")
...
export function resolveCommandContext(
  ...
  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set PAPERCLIP_COMPANY_ID, or set context profile companyId via `fightforgeai context set`.",
    );
  }

  const api = new FightForgeApiClient({
    apiBase,
    apiKey,
