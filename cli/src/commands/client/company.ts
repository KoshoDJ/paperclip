...
function shouldIncludePortableFile(filePath: string): boolean {
  ...
  const isFightForgeYaml = baseName === ".fightforge.yaml" || baseName === ".fightforge.yml";
  const contentType = binaryContentTypeByExtension[path.extname(baseName).toLowerCase()];
  return isMarkdown || isFightForgeYaml || Boolean(contentType);
}

function findPortableExtensionPath(files: Record<string, CompanyPortabilityFileEntry>): string | null {
  if (files[".fightforge.yaml"] !== undefined) return ".fightforge.yaml";
  if (files[".fightforge.yml"] !== undefined) return ".fightforge.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.fightforge.yaml") || entry.endsWith("/.fightforge.yml")) ?? null;
}
...
async function promptForImportSelection(preview: CompanyPortabilityPreviewResult): Promise<string[]> {
  ...
  while (true) {
    const choice = await p.select<ImportSelectableGroup | "company" | "confirm">({
      message: "Select what FightForge should import",
      options: [
        {
          value: "company",
...
export function registerCompanyCommands(program: Command): void {
          ...
          printOutput(
            {
              ...
              fightforgeExtensionPath: exported.fightforgeExtensionPath,
  ...
  addCommonClientOptions(
    company
      ...
      .option("--fightforge-url <url>", "Alias for --api-base on this command")
      ...
      .action(async (fromPathOrUrl: string, opts: CompanyImportOptions) => {
        try {
          if (!opts.apiBase?.trim() && opts.fightforgeUrl?.trim()) {
            opts.apiBase = opts.fightforgeUrl.trim();
          }
          const ctx = resolveCommandContext(opts);
