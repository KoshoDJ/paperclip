...
export const DEFAULT_WORKTREE_HOME = "~/.fightforge-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".fightforge");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: FightForgeConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "fightforge",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    FIGHTFORGE_HOME: paths.homeDir,
    FIGHTFORGE_INSTANCE_ID: paths.instanceId,
    PAPERCLIP_CONFIG: paths.configPath,
    PAPERCLIP_CONTEXT: paths.contextPath,
    FIGHTFORGE_IN_WORKTREE: "true",
    ...(branding?.name ? { FIGHTFORGE_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { FIGHTFORGE_WORKTREE_COLOR: branding.color } : {}),
  };
}
