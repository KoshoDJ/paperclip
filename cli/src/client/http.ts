...
export class FightForgeApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-fightforge-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the FightForge API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the FightForge server is not running, the configured URL is wrong, or the request is being blocked before it reaches FightForge.",
    "",
    "Try:",
    "- Start FightForge with `pnpm dev` or `pnpm fightforgeai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If FightForge is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PAPERCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}
