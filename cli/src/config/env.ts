...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# FightForge environment variables",
    "# Generated by FightForge CLI commands",
...
export function resolveFightForgeEnvFile(configPath?: string): string {
...
export function loadFightForgeEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeFightForgeEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readFightForgeEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeFightForgeEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeFightForgeEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readFightForgeEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeFightForgeEnvEntries(next, filePath);
  return next;
}
