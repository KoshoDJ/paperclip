...
const DATA_DIR_OPTION_HELP =
  "FightForge data directory root (isolates state from ~/.fightforge)";

program
  .name("fightforgeai")
  .description("FightForge CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadFightForgeEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start FightForge immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your FightForge setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "fightforge")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run FightForge")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the FightForge server API")
