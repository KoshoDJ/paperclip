...
import { ApiConnectionError, ApiRequestError, FightForgeApiClient } from "../client/http.js";

describe("FightForgeApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new FightForgeApiClient({
    ...
    expect(headers["x-fightforge-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new FightForgeApiClient({ apiBase: "http://localhost:3100" });
  ...
  it("throws ApiConnectionError with recovery guidance when fetch fails", async () => {
    ...
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /pnpm dev|pnpm fightforgeai run/,
    );
  });
