...
function buildSourceConfig(): FightForgeConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("fightforge-pr-432")).toBe(
      path.resolve(os.homedir(), "fightforge-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("fightforge/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://fightforge.example", 3110)).toBe("https://fightforge.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/fightforge-feature",
      homeDir: "/tmp/fightforge-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/fightforge-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/fightforge-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.FIGHTFORGE_HOME).toBe(path.resolve("/tmp/fightforge-worktrees"));
    expect(env.FIGHTFORGE_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.FIGHTFORGE_IN_WORKTREE).toBe("true");
    expect(env.FIGHTFORGE_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.FIGHTFORGE_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export FIGHTFORGE_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fightforge-worktree-secrets-"));
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fightforge-worktree-jwt-"));
    ...
    try {
      ...
      await worktreeInitCommand({
        ...
        home: path.join(tempRoot, ".fightforge-worktrees"),
      });

      const envPath = path.join(repoRoot, ".fightforge", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("PAPERCLIP_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("FIGHTFORGE_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/FIGHTFORGE_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fightforge-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".fightforge-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".fightforge", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local FightForge config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fightforge-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".fightforge", "config.json");
    const originalCwd = process.cwd();
    const originalFightForgeConfig = process.env.PAPERCLIP_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalFightForgeConfig === undefined) {
        delete process.env.PAPERCLIP_CONFIG;
      } else {
        process.env.PAPERCLIP_CONFIG = originalFightForgeConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fightforge-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/fightforge",
        targetRepoRoot: "/Users/example/fightforge-pr-432",
        workspaceCwd: "/Users/example/fightforge",
      }),
    ).toBe("/Users/example/fightforge-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/fightforge",
        targetRepoRoot: "/Users/example/fightforge-pr-432",
        workspaceCwd: "/Users/example/fightforge/packages/db",
      }),
    ).toBe("/Users/example/fightforge-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fightforge-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fightforge-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "fightforge-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("fightforge-make-test", {
        seed: false,
        home: path.join(tempRoot, ".fightforge-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".fightforge", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".fightforge", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
